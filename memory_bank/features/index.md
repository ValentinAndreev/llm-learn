...
Каждый `reviews/<stage>.md` использует единый формат:
...
Значения `Статус`:
- `advisory` — review note справочный, следующая стадия разрешена
- `blocking` — переход к следующей стадии запрещён до `fix review: <id> <stage>` и повторного review
