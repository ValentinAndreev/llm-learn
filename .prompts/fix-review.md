...
Ожидай, что `reviews/<stage>.md` использует единый формат:
- `Фича`
- `Стадия`
- `Статус` (`advisory` или `blocking`)
- `Дата`
- `## Итог`
- `## Замечания`
- `## Следующий шаг`
...
Смысл статусов:
- `advisory` — исправления необязательны и выполняются только если пользователь явно попросил `fix review: <id> <stage>`
- `blocking` — следующая стадия workflow должна ждать исправлений и повторного review
...
- Ориентируйся в первую очередь на `## Замечания`; `## Итог` и `## Следующий шаг` нужны как контекст, а не как источник новых требований.
