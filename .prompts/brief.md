# Прайминг: Brief

## Контекст для агента перед работой с Brief

Прочитай:
1. `memory_bank/project/overview.md` — рамка продукта и что НЕ строим
2. `memory_bank/project/glossary.md` — термины
3. `memory_bank/features/index.md` — существующие фичи (избегать дублирования scope)

---

## Fail-Fast Preconditions

- Для `brief: <идея>` дополнительных входных артефактов нет.
- Для `review brief: <id>` обязателен существующий `memory_bank/features/<id>_<name>/brief.md`.
- Если обязательный артефакт отсутствует, остановись и верни blocker-сообщение:
  `BLOCKER: missing memory_bank/features/<id>_<name>/brief.md. Cannot run review brief: <id>. Next step: create or restore the Brief first.`
- Не пытайся делать review brief по `spec.md`, `plan.md` или `current-focus.md`, если самого brief нет.


## Создание Brief

Помоги создать Brief для новой фичи по шаблону:

```markdown
## Цель
[1-2 предложения: что хотим получить и зачем. НЕ содержит решения — только проблему и желаемый результат]

## Для кого
[Роль пользователя, сценарий использования]

## Доменный контекст
[Термины, бизнес-процесс, ограничения из предметной области]

## Текущее состояние
[Существующая архитектура, что уже есть]

## Требования
[Конкретные требования, сценарии, ограничения]

## Что НЕ в скоупе
[Явно указать, что не делаем]
```

---

## Ревью Brief

Ты — ревьюер бизнес-задач. Проверь Brief на полноту и однозначность.

Сохрани результат ревью в `memory_bank/features/<id>_<name>/reviews/brief.md`.
Если это повторное ревью после исправлений, перезапиши тот же файл; историю сохранит git.

Используй единый формат review note:

```md
# Review

Фича: <id>_<name>
Стадия: brief
Статус: advisory | blocking
Дата: YYYY-MM-DD

## Итог
[Короткий вывод о готовности brief]

## Замечания
1. [Замечание]

## Следующий шаг
[Либо `Можно продолжать: spec: <id>`, либо `Исправить замечания: fix review: <id> brief` и затем `review brief: <id>`]
```

Статусы:
- `advisory` — brief пригоден для движения дальше; замечания могут быть необязательными или отсутствовать совсем
- `blocking` — brief не готов к следующей стадии до исправлений и повторного review

Критерии:
1. Проблема конкретна и измерима (не «улучшить», а конкретная боль или метрика)
2. Назван стейкхолдер или пользователь
3. Понятен контекст: откуда задача, почему важна сейчас
4. Brief НЕ содержит решения — только проблему и желаемый результат
5. Нет двусмысленных формулировок («быстро», «удобно», «при необходимости»)

Для каждого замечания:
- Что именно не так (цитата)
- Почему это проблема
- Как исправить (конкретное предложение)

Если замечаний нет:
- используй `Статус: advisory`
- в `## Замечания` напиши `0 замечаний.`
- в `## Следующий шаг` напиши `Можно продолжать: spec: <id>`
